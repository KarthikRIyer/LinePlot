...
public class SVGRenderer{
  ...
  public func draw_transformed_line_svg(_ x : [Float], _ y : [Float], _ thickness : Float, _ r : Float = 0, _ g : Float = 0, _ b : Float = 0, _ a : Float = 1) {
      ...
      y0 = height - y0
      y1 = height - y1
      let line = " <line x1=\"\(x0)\" y1=\"\(y0)\" x2=\"\(x1)\" y2=\"\(y1)\" style=\"stroke:rgb(\(r),\(g),\(b));stroke-width:\(thickness)\" />"
      image = image + "\n" + line
  }

  public func draw_transformed_text_svg(_ s : String, _ x : Float, _ y : Float, _ size : Float, _ thickness : Float){
    let x1 = x + 0.1*width
    let y1 = height - y + 0.1*height
    let text = "<text x=\"\(x1)\" y=\"\(y1)\" fill=\"black\">\(s)</text>"
    image = image + "\n" + text
  }

  public func draw_text_svg(_ s : String, _ x : Float, _ y : Float, _ size : Float, _ thickness : Float){
    let y1 = height - y
    let text = "<text x=\"\(x)\" y=\"\(y1)\" fill=\"black\">\(s)</text>"
    image = image + "\n" + text
  }
